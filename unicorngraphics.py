...
class SquareImage(object):
    ...
    def to_bmp(self):
        ...
        out = bytes()
        out += struct.pack("<2s6I2H6I", "BM".encode('utf-8'), total_size, 0, 54, 40, self.size, self.size, 1, 24, 0, bitmap_data_size, 2835, 2835, 0, 0)
        for line in reversed(self._image):
            out += scanline_struct.pack(*(val for col in line for val in reversed(col)))
        return out
