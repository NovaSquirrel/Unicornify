...
import functools
from unicorngraphics import hls_to_rgb
...
class Ball(object):
    ...
    def draw(self, image, worldview):
        x, y = list(map(sum, list(zip(worldview.shift, self.twoD()))))
        r = self.radius
...
class Bone(object):
    ...
    def draw(self, image, worldview, xfunc = identity, yfunc = identity):
        ...
        x1, y1 = list(map(sum, list(zip(self[0].twoD(), worldview.shift))))
        x2, y2 = list(map(sum, list(zip(self[1].twoD(), worldview.shift))))
        steps = max(*list(map(abs, (x2-x1, y2-y1))))
